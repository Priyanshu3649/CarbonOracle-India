...
class ObjectDetectionHelper @Inject constructor(
    ...
    val geminiDetector: GeminiVisionDetector
) {
    ...
    fun processFrame(bitmap: Bitmap, sessionId: String, frameIndex: Long): List<DetectedObject> {
        ...
        // Run AI / Vision tree analysis
        val treeDetections = geminiDetector.detectTrees(bitmap)
        val combinedDetections = (result.detections + treeDetections).distinctBy { it.label + it.boundingBox.toString() }
