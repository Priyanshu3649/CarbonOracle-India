...
class YOLOv8Detector @Inject constructor(private val context: Context) {
    ...
    private var demoLabels: List<String> = listOf(
        "tree", "person", "car", "tree", "dog", "cat", "bicycle", "bottle", "chair",
        "laptop", "phone", "tree", "book", "cup", "backpack", "tv", "clock"
    )
    ...
    private fun decodeAndNms(rawOutput: FloatArray, imgWidth: Int, imgHeight: Int): List<Detection> {
        ...
        for (a in 0 until NUM_ANCHORS) {
            ...
            // Find best class (applying Sigmoid to raw model logits)
            var bestClass = -1
            var bestScore = confidenceThreshold
            for (c in 0 until numClasses) {
                val rawScore = rawOutput[(4 + c) * NUM_ANCHORS + a]
            ...
            // Normalize coordinate range [0, 1]
            val normCx = if (cx <= 1.0f) cx else cx / INPUT_SIZE
            val normCy = if (cy <= 1.0f) cy else cy / INPUT_SIZE
            val normW  = if (w  <= 1.0f) w  else w  / INPUT_SIZE
            val normH  = if (h  <= 1.0f) h  else h  / INPUT_SIZE

            val x1 = (normCx - normW / 2f) * imgWidth
            val y1 = (normCy - normH / 2f) * imgHeight
            val x2 = (normCx + normW / 2f) * imgWidth
            val y2 = (normCy + normH / 2f) * imgHeight
        ...
        if (candidates.isEmpty()) {
            ...
            for (a in 0 until NUM_ANCHORS) {
                for (c in 0 until numClasses) {
                    val s = rawOutput[(4 + c) * NUM_ANCHORS + a]
                    if (s > maxSc) {
                        maxSc = s
                        maxClass = c
                    }
            ...
            Timber.d("YOLOv8 Debug: maxScore=$maxSc (class $maxClass '${labels.getOrNull(maxClass)}'), thresh=$confidenceThreshold | raw cx=${rawOutput[0]}, cy=${rawOutput[NUM_ANCHORS]}, w=${rawOutput[NUM_ANCHORS*2]}, h=${rawOutput[NUM_ANCHORS*3]}")
        }
        ...
        for ((classIdx, boxes) in byClass) {
            ...
            while (sorted.isNotEmpty() && results.size < MAX_DETECTIONS) {
                ...
                val rawLabel = labels.getOrElse(classIdx) { "unknown" }
                val displayLabel = when (rawLabel.lowercase().trim()) {
                    "potted plant", "plant", "pottedplant" -> "tree"
                    else -> rawLabel
                }
                results.add(
                    Detection(
